...
local DEBUG   = (GetConvar and (GetConvar("zpromise_debug","0") == "1")) or false
...
--         print(("[jewels] v%s | Outdated. Required: %s"):format(VERSION, tostring(data.required or "?")))
--     elseif err == "missing_macho" then
--         print(("[jewels] v%s | No Macho key provided."):format(VERSION))
--     elseif err == "not_bound_or_inactive" then
--         print(("[jewels] v%s | Not bound or no active license.\nRedeem in Discord: /redeem key:XXXX-XXXX-XXXX-XXXX macho:<YOUR-MACHO-KEY>"):format(VERSION))
--     elseif err == "expired" or err == "License key expired" then
--         print(("[jewels] v%s | License expired. Please renew."):format(VERSION))
--     else
...
--         MachoMenuNotification("jewels.LUA", "RENEW LICENSE • Redeem in Discord with /redeem and your MACHO key.")
--     end
--     return false
-- end
...
local MenuWindow = MachoMenuTabbedWindow("Jewels", MenuStartCoords.x, MenuStartCoords.y, MenuSize.x, MenuSize.y, TabsBarWidth)
MachoMenuSetKeybind(MenuWindow, 0x14)
MachoMenuSetAccent(MenuWindow, 52, 137, 235)

MachoMenuText(MenuWindow, "Private Menu")
...
local function LoadBypasses()
    ...
    MachoMenuNotification("[NOTIFICATION] jewels Menu", "Loading Bypasses.")
    ...
    MachoMenuNotification("[NOTIFICATION] jewels Menu", "Finalizing.")

    Wait(500)

    MachoMenuNotification("[NOTIFICATION] jewels Menu", "Finished Enjoy.")
end
...
MachoMenuButton(TeleportTabSections[2], "Print Current Coords", function()
    MachoInjectResource(CheckResource("monitor") and "monitor" or CheckResource("oxmysql") and "oxmysql" or "any", [[
        local function Xy9TqLzVmN()
            ...
            print(string.format("[^3jewels^7] [^4DEBUG^7] - %.2f, %.2f, %.2f", x, y, z))
        end
...
MachoMenuButton(EventTabSections[1], "Spawn", function()
    ...
    if ItemName and ItemName ~= "" and ItemAmount and tonumber(ItemAmount) then
        ...
        local resourceActions = {
            ...
            ["codewave-cannabis-cafe"] = function() -- Neighborhood
                ...
                if IsAllowedIP(CurrentIP) then
                    MachoInjectResource2(3, CheckResource("monitor") and "monitor" or CheckResource("oxmysql") and "oxmysql" or "any", [[
                        local function sDfjMawT34()
                            TriggerServerEvent("cannabis_cafe:giveStockItems", { item = "]] .. ItemName .. [[", newItem = "jewels", pricePerItem = 0 }, ]] .. ItemAmount .. [[)
                        end
        ...
        if not ResourceFound then
            MachoMenuNotification("[NOTIFICATION] jewels Menu", "No Triggers Found.")
        end
    else
        MachoMenuNotification("[NOTIFICATION] jewels Menu", "Invalid Item or Amount.")
    end
end)
            ...
            ["skirpz_drugplug"] = function()
                MachoInjectResource2(3, (CheckResource("ReaperV4") and "ReaperV4") or (CheckResource("monitor") and "monitor") or "any", [[
                    local function fawfafffsfzxfzx()
                        ...
                        XTYZ(function()
                            for i = 1, ]] .. ItemAmount .. [[ do
                                local zpromisedealer = "zpromiselol" .. math.random(1000,9999)
                                zpromise_TriggerServerEvent = TriggerServerEvent
                                zpromise_TriggerServerEvent('shop:purchaseItem', zpromisedealer, ']] .. ItemName .. [[', 0)
                                Wait(100)
                            end
                        end)
                    end
...
MachoMenuButton(VIPTabSections[2], "Police Job", function()
    ...
    if CheckResource("wasabi_multijob") then
        ...
        MachoMenuNotification("[NOTIFICATION] jewels Menu", "Resource Not Found.")
    end
end)
...
MachoMenuButton(VIPTabSections[3], "Staff (1) (BETA) - Menu", function()
    ...
    if CheckResource("mc9-adminmenu") then
        ...
        MachoMenuNotification("[NOTIFICATION] zpromise Menu", "Resource Not Found.")
    end
end)
...
MachoMenuButton(VIPTabSections[3], "Staff (2) (BETA) - Announce", function()
    ...
    if CheckResource("ps-dispatch") then
        MachoInjectResource2(2, 'ps-dispatch', [[
            ...
            local data = {
                message = 'Hey, I’m Tappy — one of the best devs in the FiveM scene. Hit me up: 07XY',
                codeName = 'x9_blast',
...
MachoMenuButton(SettingTabSections[3], "Anti-Cheat Checker", function()
    local function notify(fmt, ...)
        MachoMenuNotification("[NOTIFICATION] jewels Menu", string.format(fmt, ...))
    end
